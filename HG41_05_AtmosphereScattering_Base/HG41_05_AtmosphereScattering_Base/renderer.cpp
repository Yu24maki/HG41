...
ID3D11Buffer*			CRenderer::m_ConstantBuffer = NULL;
...
void CRenderer::Init()
{
	...
	hr = D3D11CreateDeviceAndSwapChain( NULL,
										...
										NULL,
#ifdef _DEBUG
										D3D11_CREATE_DEVICE_DEBUG,//デバッグ出力
#else
										0,
#endif // DEBUG
										NULL,
	...
	m_ImmediateContext->VSSetConstantBuffers( 1, 1, &m_MaterialBuffer );
	m_ImmediateContext->PSSetConstantBuffers( 1, 1, &m_MaterialBuffer );
...
void CRenderer::SetWorldViewProjection2D()
{
	...
	XMStoreFloat4x4(&m_Constant.WorldMatrix, XMMatrixTranspose(world));

	XMMATRIX view;
	view = XMMatrixIdentity();
	//m_ImmediateContext->UpdateSubresource(m_ViewBuffer, 0, NULL, &XMMatrixTranspose(view), 0, 0);
	XMStoreFloat4x4(&m_Constant.ViewMatrix, XMMatrixTranspose(view));

	XMMATRIX projection;
	projection = XMMatrixOrthographicOffCenterLH( 0.0f, SCREEN_WIDTH, SCREEN_HEIGHT, 0.0f, 0.0f, 1.0f );
	//m_ImmediateContext->UpdateSubresource( m_ProjectionBuffer, 0, NULL, &XMMatrixTranspose(projection), 0, 0 );
	XMStoreFloat4x4(&m_Constant.WorldMatrix, XMMatrixTranspose(projection));

	m_ImmediateContext->UpdateSubresource(m_ConstantBuffer, 0, NULL, &m_Constant, 0, 0);
}
...
void CRenderer::SetProjectionMatrix( XMMATRIX *ProjectionMatrix )
{
	...
	//m_ImmediateContext->UpdateSubresource(m_ProjectionBuffer, 0, NULL, &XMMatrixTranspose(projection), 0, 0);
	XMStoreFloat4x4(&m_Constant.ProjectionMatrix, XMMatrixTranspose(projection));
	m_ImmediateContext->UpdateSubresource(m_ConstantBuffer, 0, NULL, &m_Constant, 0, 0);

}

void CRenderer::SetCameraPosition(XMFLOAT4 *CameraPosition)
{

	m_Constant.CameraPosition = *CameraPosition;
	m_ImmediateContext->UpdateSubresource(m_ConstantBuffer, 0, NULL, &m_Constant, 0, 0);
